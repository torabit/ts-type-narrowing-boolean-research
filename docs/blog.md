# boolean変数に代入すると型の絞り込みが消える ── TypeScript型narrowingとコーディングルールの両立を考える

## はじめに

こんにちは。フロントエンドエンジニアの torabit です。
...
| ---------------------- | ------ | ---------- | ------------------ | ------------------------------------ |
| インラインチェック     | o      | x          | 不要               |                                      |
| Type Predicate         | o      | o          | 不要               | 実装の正しさはコンパイラが保証しない |
| Early Return           | o      | o          | 必要               | コンパイラがnarrowingを自動検証      |
| Non-null Assertion (!) | x      | o          | 不要               |                                      |
| 三項演算子             | o      | x          | 不要               |                                      |
